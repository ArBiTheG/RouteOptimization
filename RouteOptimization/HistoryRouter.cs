...
namespace RouteOptimization
{
    public class HistoryRouter<TViewModelBase> : Router<TViewModelBase> where TViewModelBase : class
    {
        ...
        public TViewModelBase GoTo(Type typeViewModel)
        {
            var viewModel = InstantiateViewModel<TViewModelBase>(typeViewModel);
            CurrentViewModel = viewModel;
            Push(viewModel);
            return viewModel;
        }
        private T InstantiateViewModel<T>(Type type) where T : TViewModelBase
        {
            return (T)Convert.ChangeType(CreateViewModel(type), type);
        }
